...
const Login = () => {
    ...
    const { login, logout, register, isAuthenticated, user, isLoading: authLoading } = useAuth();
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            const roleToLogin = selectedUserType === 'B2B' ? 'B2B_CUSTOMER' : selectedUserType || 'RETAIL_CUSTOMER';
            const result = await login(email, password, roleToLogin);
            if (result.success && result.user) {
                // Use user from response directly
                const role = result.user.role;
                console.log('[Login] Login successful, redirecting. Role:', role);

                // STRICT CHECK: Ensure returned role matches selected portal
                if (selectedUserType === 'ADMIN' && role !== 'ADMIN') {
                    setError('Access Denied: You cannot log in to the Admin Portal with this account.');
                    await logout(); // Clear the session immediately
                    setIsLoading(false);
                    return;
                }
                if (selectedUserType === 'DEALER' && role !== 'DEALER') {
                    setError('Access Denied: You cannot log in to the Dealer Portal with this account.');
                    await logout();
                    setIsLoading(false);
                    return;
                }
                if (selectedUserType === 'B2B' && role !== 'B2B_CUSTOMER') {
                    setError('Access Denied: You cannot log in to the B2B Portal with this account.');
                    await logout();
                    setIsLoading(false);
                    return;
                }
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            const result = await register({
                ...
                role: selectedUserType === 'B2B' ? 'B2B_CUSTOMER' : undefined,
            });
            if (result.success) {
                navigate('/dashboard', { replace: true });
            } else {
