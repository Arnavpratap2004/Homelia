...
import { ordersApi, quotesApi, invoicesApi, authApi } from '../api';
...
const Dashboard = () => {
    ...
    // Demo orders (used as fallback) - defined before useState to avoid reference error
    ...
    // Profile form - use real user data from AuthContext as default
    const [profileForm, setProfileForm] = useState(() => {
        // Always prioritize real user data from auth over hardcoded defaults
        return {
            name: user?.name || 'User',
    ...
    // Update profile form when user data changes (after login/registration)
    useEffect(() => {
        if (user) {
            setProfileForm(prev => ({
                ...prev,
                ...
                gstin: user.gstNumber || prev.gstin
            }));
        }
    }, [user]);
    ...
    // Save profile handler - persists to backend API
    const handleSaveProfile = async () => {
        setIsSavingProfile(true);
        setSaveMessage(null);

        console.log('[Dashboard] Starting profile save with data:', {
            name: profileForm.name,
            phone: profileForm.phone,
            companyName: profileForm.company,
            gstNumber: profileForm.gstin,
        });
        ...
        try {
            ...
            console.log('[Dashboard] Profile update response:', response);

            if (response.success) {
                // Refresh user context so changes reflect everywhere
                console.log('[Dashboard] Profile saved successfully, refreshing user context');
                refreshUser();
                setSaveMessage('Profile saved successfully!');
            } else {
                console.error('[Dashboard] Profile save failed:', response.message);
