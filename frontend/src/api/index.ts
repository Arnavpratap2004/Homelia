...
// const API_BASE_URL = 'https://homelia-backend-b81ca6eb78ba.herokuapp.com/api';
...
export const authApi = {
    async login(email: string, password: string, role: string = 'RETAIL_CUSTOMER'): Promise<AuthResponse> {
        const endpoint = '/auth/login';

        const response = await apiRequest<AuthResponse['data']>(endpoint, {
            method: 'POST',
...
export const ordersApi = {
    ...
    async list(params?: {
        ...
        const query = searchParams.toString();
        // Use /orders for shared list (accessible to both user and admin)
...
export const quotesApi = {
    ...
    async list(params?: {
        ...
        const query = searchParams.toString();
        // Use /quotes for shared list (accessible to both user and admin)
        return apiRequest(`/quotes${query ? `?${query}` : ''}`);
    },
    ...
    async approve(id: string): Promise<ApiResponse<unknown>> {
        return apiRequest(`/admin/quotes/${id}/approve`, {
            method: 'PATCH',
        });
    },
    ...
    async convertToOrder(id: string, data: {
        shippingAddress: any;
        billingAddress: any;
    }): Promise<ApiResponse<unknown>> {
        return apiRequest(`/quotes/${id}/convert`, {
            method: 'POST',
            body: JSON.stringify(data),
        });
    },
...
export const creditApi = {
    async request(amount: number, notes?: string): Promise<ApiResponse<unknown>> {
        return apiRequest('/credit/request', {
            method: 'POST',
            body: JSON.stringify({ amount, notes }),
        });
    },

    async getMyRequests(): Promise<ApiResponse<unknown>> {
        return apiRequest('/credit/my-requests');
    },

    // Admin
    async getAllRequests(status?: string): Promise<ApiResponse<unknown>> {
        const query = status ? `?status=${status}` : '';
        return apiRequest(`/credit/admin/requests${query}`);
    },

    async respond(id: string, status: 'APPROVED' | 'REJECTED', adminNotes?: string): Promise<ApiResponse<unknown>> {
        return apiRequest(`/credit/admin/requests/${id}/respond`, {
            method: 'PATCH',
            body: JSON.stringify({ status, adminNotes }),
        });
    },
...
export const invoicesApi = {
    ...
    async createManual(data: any): Promise<ApiResponse<unknown>> {
        return apiRequest('/invoices/manual', {
            method: 'POST',
            body: JSON.stringify(data),
        });
    },
...
export const usersApi = {
    ...
    async verify(id: string): Promise<ApiResponse<User>> {
...
export const notificationsApi = {
    async list(params?: { limit?: number }): Promise<ApiResponse<any[]>> {
