...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // Login function
    const login = async (email: string, password: string, role: string = 'RETAIL_CUSTOMER'): Promise<{ success: boolean; error?: string; user?: User }> => {
        try {
            ...
            // Propagate the specific error message from the backend (e.g. "Access Denied")
    ...
    const register = async (data: RegisterData): Promise<{ success: boolean; error?: string }> => {
        try {
            ...
            if (error instanceof Error) {
                ...
                // Check for common duplicate entry errors
                if (message.toLowerCase().includes('phone') && message.toLowerCase().includes('already')) {
    ...
    // Refresh user from API (fetches fresh data including creditLimit)
    const refreshUser = async () => {
        try {
            ...
            if (response.success && response.data) {
                ...
                setUser(freshUser);
                console.log('[AuthContext] User refreshed from API, creditLimit:', freshUser.creditLimit);
            }
        } catch (error) {
