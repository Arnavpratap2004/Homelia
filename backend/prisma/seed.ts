...
async function main() {
    ...
    const admin = await prisma.user.upsert({
        ...
        update: { passwordHash: adminPassword, isActive: true }, // Force update password AND ensure active
        create: {
    ...
    const savedUser = await prisma.user.findUnique({ where: { email: 'admin@homelia.in' } });
    const passwordWorks = savedUser ? await bcrypt.compare('admin123', savedUser.passwordHash) : false;
    console.log('✅ Admin user created:', admin.email, '| isActive:', savedUser?.isActive, '| Password test:', passwordWorks);
