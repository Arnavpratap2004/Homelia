...
class AuthService {
    ...
    async login(input: LoginInput): Promise<{ user: UserResponse; tokens: AuthTokens }> {
        console.log('[DEBUG] Login attempt for:', input.email);
        // Find user
        ...
        // STRICT ROLE CHECK (Prevents Admin from logging into Dealer portal, etc.)
        if (input.role && user.role !== input.role) {
            console.log(`[DEBUG] Role mismatch. Expected: ${input.role}, Actual: ${user.role}`);
        ...
        console.log('[DEBUG] Password verified. Generating tokens...');
        ...
        console.log('[DEBUG] DB updated. Invalidating cache...');

        // SECURITY: Invalidate any existing session cache to prevent fixation
        try {
            await import('../config/index.js').then(m => m.cacheDelete(`session:${user.id}`));
            console.log('[DEBUG] Cache invalidated.');
        } catch (e) {
            console.error('[DEBUG] Cache invalidation failed (non-fatal):', e);
        }
    ...
    async refreshTokens(refreshToken: string): Promise<AuthTokens> {
        ...
        // SECURITY: Invalidate cache to force re-fetch of fresh permissions relative to new token
        await import('../config/index.js').then(m => m.cacheDelete(`session:${user.id}`));
    ...
    async logout(userId: string): Promise<void> {
        ...
        // SECURITY: Clear session cache
        await import('../config/index.js').then(m => m.cacheDelete(`session:${userId}`));
    }
