...
router.get('/users',
    validateQuery(z.object({
        ...
        isVerified: z.string().optional(), // 'true' or 'false'
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        ...
        const where: any = {};
        if (req.query.role) where.role = req.query.role as never;
...
router.patch('/orders/:id/status',
    ...
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        ...
        const result = await orderService.updateStatus(req.params.id, status, adminNotes);
        res.json({
            success: true,
...
// PATCH /api/admin/quotes/:id/approve - Approve quote
router.patch('/quotes/:id/approve',
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        const result = await quoteService.approve(req.params.id);
        res.json({
            success: true,
...
// PATCH /api/admin/quotes/:id/reject - Reject quote
router.patch('/quotes/:id/reject',
    asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
        const { reason } = req.body;
        const result = await quoteService.reject(req.params.id, reason);
        res.json({
            success: true,
